...
require('./lib/d3.js');
require('./lib/d3-geo-projection.js');
require('./lib/d3.geo.zoom.js');
...
Celestial.display = function (config) {
  ...
  var margin = [16, 16],
    width = getWidth(),
    canvaswidth = isNumber(cfg.background.width)
      ? width + cfg.background.width
      : width,
    pixelRatio = window.devicePixelRatio || 1,
    projectionSetting = getProjection(cfg.projection, cfg.projectionRatio);
  console.log('cfg.projection', cfg.projection);
  console.log('🚀 ~ projectionSetting:', projectionSetting);

  if (!projectionSetting) return;

  if (cfg.lines.graticule.lat && cfg.lines.graticule.lat.pos[0] === 'outline')
    projectionSetting.scale -= 2;
  ...
  mapProjection = Celestial.projection(cfg.projection)
    .rotate(rotation)
  ...
  //   .scaleExtent([scale, scale * zoomextent])
  //   .on('zoom.redraw', redraw);
  // Set initial zoom level
  scale *= zoomlevel;

  var canvas = d3.select(parentElement).selectAll('canvas'),
    culture = cfg.culture !== '' && cfg.culture !== 'iau' ? cfg.culture : '';
  if (canvas._groups[0].length === 0)
    canvas = d3.select(parentElement).append('canvas');
  //canvas.attr("width", width).attr("height", height);
  canvas
    .style('width', px(canvaswidth))
    .style('height', px(canvasheight))
    .attr('width', canvaswidth * pixelRatio)
  ...
  circle = d3.geoCircle().center([0, 0]).radius([90]);
  daylight = d3.geoCircle().center([0, 0]).radius([179.9]);
  ...
  if ($('loc') === null) geo(cfg);
  else if (cfg.location === true && cfg.follow === 'zenith')
    rotate({center: Celestial.zenith()});
  ...
  function load() {
    ...
    for (var key in cfg.lines) {
      ...
      if (key === 'graticule') {
        ...
        if (
          has(cfg.lines.graticule, 'lon') &&
          cfg.lines.graticule.lon.pos.length > 0
        )
          container
            .selectAll(parentElement + ' .gridvalues_lon')
            .data(getGridValues('lon', cfg.lines.graticule.lon.pos))
            .enter()
            .append('path')
            .attr('class', 'graticule_lon');
        if (
          has(cfg.lines.graticule, 'lat') &&
          cfg.lines.graticule.lat.pos.length > 0
        )
          container
            .selectAll(parentElement + ' .gridvalues_lat')
        ...
        container
          .append('path')
          .datum(
            d3.geoCircle().center([0, 0]).radius([90]),
            // .origin(transformDeg(poles[key], euler[cfg.transform])),
          )
          .attr('class', key);
      }
    }
    ...
    d3.json(path + 'mw.json', function (error, json) {
      ...
      container
        .selectAll(parentElement + ' .mway')
        .data(mw.features)
        .enter()
        .append('path')
        .attr('class', 'mw');
      container
        .selectAll(parentElement + ' .mwaybg')
    ...
    d3.json(path + filename('constellations'), function (error, json) {
      ...
      container
        .selectAll(parentElement + ' .constnames')
        .data(con.features)
        .enter()
        .append('text')
    ...
    d3.json(
      path + filename('constellations', 'borders'),
      function (error, json) {
        if (error) return console.warn(error);
        ...
        container
          .selectAll(parentElement + ' .bounds')
          .data(cb.features)
          .enter()
          .append('path')
    ...
    d3.json(path + filename('planets'), function (error, json) {
      ...
      container
        .selectAll(parentElement + ' .planets')
        .data(pl)
        .enter()
        .append('path')
        .attr('class', 'planet');
  ...
  function reproject(config) {
    ...
    var rot = mapProjection.rotate(),
      ...
      ext = zoom.scaleExtent(),
      clip = [],
      prjFrom = Celestial.projection(cfg.projection)
        .center(ctr)
        .translate([canvaswidth / 2, canvasheight / 2])
        .scale([ext[0]]),
    ...
    var prjTo = Celestial.projection(config.projection)
      .center(ctr)
      .translate([canvaswidth / 2, canvaswidth / prj.ratio / 2])
    ...
    d3.select({})
      ...
      .tween('projection', function () {
        ...
        canvasheight = isNumber(cfg.background.width)
          ? height + cfg.background.width
          : height;
        scale = (projectionSetting.scale * width) / 1024;
        //canvas.attr("width", width).attr("height", height);
        canvas
        ...
        mapProjection = Celestial.projection(config.projection)
          .rotate(rot)
          .translate([canvaswidth / 2, canvasheight / 2])
          .scale(scale * zoomlevel);
        map.projection(mapProjection);
  ...
  function redraw() {
    ...
    if (has(cfg.lines.graticule, 'lon')) {
      ...
      container
        .selectAll(parentElement + ' .graticule_lon')
        .each(function (d, i) {
    ...
    if (has(cfg.lines.graticule, 'lat')) {
      ...
      container
        .selectAll(parentElement + ' .graticule_lat')
        .each(function (d, i) {
          if (clip(d.geometry.coordinates)) {
            var pt = mapProjection(d.geometry.coordinates);
            gridOrientation(pt, d.properties.orientation);
    ...
    if (cfg.stars.show) {
      ...
      container.selectAll(parentElement + ' .star').each(function (d) {
        if (
          clip(d.geometry.coordinates) &&
          d.properties.mag <= cfg.stars.limit
        ) {
          var pt = mapProjection(d.geometry.coordinates),
            r = starSize(d);
          ...
          context.fill();
          if (
            cfg.stars.designation &&
            d.properties.mag <= cfg.stars.designationLimit * adapt
          ) {
            setTextStyle(cfg.stars.designationStyle);
            context.fillText(starDesignation(d.id), pt[0] + r, pt[1]);
          }
          if (
            cfg.stars.propername &&
            d.properties.mag <= cfg.stars.propernameLimit * adapt
          ) {
    ...
    if (cfg.dsos.show) {
      container.selectAll(parentElement + ' .dso').each(function (d) {
        if (
          clip(d.geometry.coordinates) &&
          dsoDisplay(d.properties, cfg.dsos.limit)
        ) {
          var pt = mapProjection(d.geometry.coordinates),
          ...
          if (
            cfg.dsos.names &&
            dsoDisplay(d.properties, cfg.dsos.nameLimit * adapt)
          ) {
            setTextStyle(cfg.dsos.nameStyle);
            if (cfg.dsos.colors === true)
    ...
    if (
      (cfg.location || cfg.formFields.location) &&
      cfg.planets.show &&
      Celestial.origin
    ) {
      ...
      container.selectAll(parentElement + ' .planet').each(function (d) {
        ...
        if (clip(pos)) {
          ...
          if (cfg.planets.symbolType === 'letter') {
            ...
            r =
              has(sym, 'size') && isNumber(sym.size)
                ? sym.size * adapt
                : planetSize(p.ephemeris);
            context.fillStyle = sym.fill;
            Canvas.symbol()
              .type('circle')
              .size(r * r)
    ...
    if (
      ...
      if (sol) {
        ...
        setSkyStyle(dist, pt);
        container
          .selectAll(parentElement + ' .daylight')
          .datum(daylight)
          .attr('d', map);
        context.fill();
    ...
    if (
      ...
      container
        .selectAll(parentElement + ' .horizon')
        .datum(circle)
        .attr('d', map);
      context.fill();
  ...
  function clip(coords) {
    return projectionSetting.clip && d3.geoDistance(cfg.center, coords) > halfπ
      ? 0
      : 1;
  }
  ...
  function setStyle(s) {
    ...
    if (has(s, 'dash')) context.setLineDash(s.dash);
  ...
  function dsoDisplay(prop, limit) {
    return (
      (prop.mag === 999 && Math.sqrt(parseInt(prop.dim)) > limit) ||
  ...
  function dsoSize(prop) {
    if (!prop.mag || prop.mag === 999)
      return Math.pow((parseInt(prop.dim) * dsobase * adapt) / 7, 0.5);
...
Celestial.projection = function (projection) {
  console.log('🚀 ~ projection:', projection, d3.geo);
  var p, raw, forward;
  ...
  console.log('d3.geoProjection(forward)', d3.geoProjection(forward));
...
Celestial.add = function (dat) {
  ...
  if (
    (dat.type === 'dso' || dat.type === 'json') &&
    (!has(dat, 'file') || !has(dat, 'callback'))
  )
    return console.log("Can't add data file");
  if ((dat.type === 'line' || dat.type === 'raw') && !has(dat, 'callback'))
    return console.log("Can't add data");
...
function getPlanets(d) {
  ...
  for (var key in d) {
    ...
    if (key === 'ter') Celestial.origin = dat;
    else res.push(dat);
  }
  ...
  for (var i = 0; i < loc.length; i++) {
    switch (loc[i]) {
      case 'center':
        ...
        else lines = lines.concat(getLine(type, cfg.center[1], 'S'));
        break;
      case 'outline':
        if (type === 'lon') {
      ...
      default:
        if (isNumber(loc[i])) {
          if (type === 'lat') lines = lines.concat(getLine(type, loc[i], 'N'));
          else lines = lines.concat(getLine(type, loc[i], 'S'));
          break;
        }
    }
  }
...
var settings = {
  ...
  constellations: {
    ...
    nameStyle: {
      ...
      opacity: 0.8,
      font: [
        "14px 'Lucida Sans Unicode', 'DejaVu Sans', Helvetica, Arial, sans-serif", // Different fonts for brighter &
  ...
  set: function (cfg) {
    ...
    res.constellations.nameStyle.font = arrayfy(
      res.constellations.nameStyle.font,
    );
    res.constellations.nameStyle.opacity = arrayfy(
      res.constellations.nameStyle.opacity,
    );
    res.constellations.nameStyle.fill = arrayfy(
      res.constellations.nameStyle.fill,
    );
    res.constellations.lineStyle.width = arrayfy(
      res.constellations.lineStyle.width,
    );
    res.constellations.lineStyle.opacity = arrayfy(
      res.constellations.lineStyle.opacity,
    );
    res.constellations.lineStyle.stroke = arrayfy(
      res.constellations.lineStyle.stroke,
    );
  ...
  applyDefaults: function (cfg) {
    ...
    if (cfg) {
      ...
      if (has(cfg, 'stars')) {
        ...
        if (has(cfg.stars, 'namelimit'))
          res.stars.designationLimit = cfg.stars.namelimit;
        if (has(cfg.stars, 'namestyle'))
          Object.assign(res.stars.designationStyle, cfg.stars.namestyle);
        // proper -> propername
        if (has(cfg.stars, 'proper')) res.stars.propername = cfg.stars.proper;
        if (has(cfg.stars, 'propernamelimit'))
          res.stars.propernameLimit = cfg.stars.propernamelimit;
        if (has(cfg.stars, 'propernamestyle'))
          Object.assign(res.stars.propernameStyle, cfg.stars.propernamestyle);
      }

      if (has(cfg, 'dsos')) {
        // names, desig -> namesType
        //if (has(cfg.dsos, "names") && cfg.dsos.names === true) res.dsos.namesType = "name";
        if (has(cfg.dsos, 'desig') && cfg.dsos.desig === true)
          res.dsos.namesType = 'desig';
        if (has(cfg.dsos, 'namelimit')) res.dsos.nameLimit = cfg.dsos.namelimit;
        if (has(cfg.dsos, 'namestyle'))
          Object.assign(res.dsos.nameStyle, cfg.dsos.namestyle);
      }

      if (has(cfg, 'constellations')) {
        // names, desig -> namesType
        if (has(cfg.constellations, 'show') && cfg.constellations.show === true)
          res.constellations.names = true;
        //if (has(cfg.constellations, "names") && cfg.constellations.names === true) res.constellations.namesType = "name";
        if (
          has(cfg.constellations, 'desig') &&
          cfg.constellations.desig === true
        )
          res.constellations.namesType = 'desig';
        if (res.constellations.namesType === 'latin')
          res.constellations.namesType = 'la';
        if (res.constellations.namesType === 'iau')
          res.constellations.namesType = 'name';
        if (has(cfg.constellations, 'namestyle'))
          Object.assign(
            res.constellations.nameStyle,
            cfg.constellations.namestyle,
          );
        if (has(cfg.constellations, 'linestyle'))
          Object.assign(
            res.constellations.lineStyle,
            cfg.constellations.linestyle,
          );
        if (has(cfg.constellations, 'boundstyle'))
          Object.assign(
            res.constellations.boundStyle,
            cfg.constellations.boundstyle,
          );
      }

      if (has(cfg, 'planets')) {
        if (has(cfg.planets, 'style'))
          Object.assign(res.planets.style, cfg.planets.symbolStyle);
      }
    }
    //Assign default name types if none given
    if (!res.stars.designationType || res.stars.designationType === '')
      res.stars.designationType = 'desig';
    if (
      !has(
        formats.starnames[res.culture].designation,
        res.stars.designationType,
      )
    )
      res.designationType = 'desig';
    if (!res.stars.propernameType || res.stars.propernameType === '')
      res.stars.propernameType = 'name';
    if (
      !has(formats.starnames[res.culture].propername, res.stars.propernameType)
    )
      res.propernameType = 'name';
    if (!res.dsos.namesType || res.dsos.namesType === '')
      res.dsos.namesType = 'desig';
    if (!res.constellations.namesType || res.constellations.namesType === '')
      res.constellations.namesType = 'desig';
    if (
      !has(
        formats.constellations[res.culture].names,
        res.constellations.namesType,
      )
    )
      res.constellations.namesType = 'name';
    if (!res.planets.symbolType || res.planets.symbolType === '')
      res.planets.symbolType = 'symbol';
    if (!res.planets.namesType || res.planets.namesType === '')
      res.planets.namesType = 'desig';
    if (!has(formats.planets[res.culture].names, res.planets.namesType))
      res.planets.namesType = 'desig';
...
var projections = {
  ...
  geoAitoffRaw: {n: 'Aitoff', arg: null, scale: 162},
  geoArmadilloRaw: {n: 'Armadillo', arg: 0, scale: 250},
  geoAugustRaw: {n: 'August', arg: null, scale: 94, ratio: 1.4},
  ...
  geoBakerRaw: {n: 'Baker Dinomic', arg: null, scale: 160, ratio: 1.4},
  geoBerghausRaw: {
    n: 'Berghaus Star',
    arg: 0,
    scale: 320,
  ...
  geoBoggsRaw: {n: 'Boggs Eumorphic', arg: null, scale: 170},
  geoBonneRaw: {n: 'Bonne', arg: Math.PI / 2.5, scale: 225, ratio: 0.88},
  geoBromleyRaw: {n: 'Bromley', arg: null, scale: 162},
  //  "butterfly"Raw: {n:"Butterfly", arg:null, scale:31, ratio:1.1, clip:true},
  geoCassiniRaw: {n: 'Cassini', arg: null, scale: 325, ratio: 1.0, clip: true},
  geoCollignonRaw: {n: 'Collignon', arg: null, scale: 100, ratio: 2.6},
  geoCraigRaw: {
  ...
  geoCrasterRaw: {n: 'Craster Parabolic', arg: null, scale: 160},
  ...
  geoEckert1Raw: {n: 'Eckert I', arg: null, scale: 175},
  geoEckert2Raw: {n: 'Eckert II', arg: null, scale: 175},
  geoEckert3Raw: {n: 'Eckert III', arg: null, scale: 190},
  geoEckert4Raw: {n: 'Eckert IV', arg: null, scale: 190},
  geoEckert5Raw: {n: 'Eckert V', arg: null, scale: 182},
  geoEckert6Raw: {n: 'Eckert VI', arg: null, scale: 182},
  geoEisenlohrRaw: {n: 'Eisenlohr', arg: null, scale: 102},
  geoEquirectangularRaw: {n: 'Equirectangular', arg: null, scale: 165},
  geoFaheyRaw: {n: 'Fahey', arg: null, scale: 196, ratio: 1.4},
  ...
  geoFoucautRaw: {n: 'Foucaut', arg: null, scale: 142},
  geoGinzburg4Raw: {n: 'Ginzburg IV', arg: null, scale: 180, ratio: 1.7},
  geoGinzburg5Raw: {n: 'Ginzburg V', arg: null, scale: 196, ratio: 1.55},
  geoGinzburg6Raw: {n: 'Ginzburg VI', arg: null, scale: 190, ratio: 1.4},
  geoGinzburg8Raw: {n: 'Ginzburg VIII', arg: null, scale: 205, ratio: 1.3},
  geoGinzburg9Raw: {n: 'Ginzburg IX', arg: null, scale: 190, ratio: 1.4},
  //"guyou"Raw: {n:"Guyou", arg:null, scale:160, ratio:2, clip:true},
  //"bonne"Raw: {n:"Heart", arg:Math.PI/2.5, scale:225, ratio:0.88},
  geoHomolosineRaw: {n: 'Goode Homolosine', arg: null, scale: 160, ratio: 2.2},
  geoHammerRaw: {n: 'Hammer', arg: 2, scale: 180},
  geoHatanoRaw: {n: 'Hatano', arg: null, scale: 186},
  geoHealpixRaw: {n: 'HEALPix', arg: 1, scale: 320, ratio: 1.2},
  geoHillRaw: {n: 'Hill Eucyclic', arg: 2, scale: 190, ratio: 1.1},
  geoKavrayskiy7Raw: {n: 'Kavrayskiy VII', arg: null, scale: 185, ratio: 1.75},
  geoLagrangeRaw: {
    n: 'Lagrange',
    arg: Math.PI / 4,
    scale: 88,
  ...
  geoLarriveeRaw: {n: "l'Arrivée", arg: null, scale: 160, ratio: 1.25},
  geoLaskowskiRaw: {
  ...
  geoLoximuthalRaw: {n: 'Loximuthal', arg: Math.PI / 4, scale: 175, ratio: 1.8},
  geoMercatorRaw: {n: 'Mercator', arg: null, scale: 160, ratio: 1.3},
  geoMillerRaw: {n: 'Miller', arg: null, scale: 160, ratio: 1.5},
  geoMollweideRaw: {n: 'Mollweide', arg: null, scale: 180},
  geoNaturalEarthRaw: {n: 'Natural Earth', arg: null, scale: 185, ratio: 1.85},
  geoNellHammerRaw: {n: 'Nell Hammer', arg: null, scale: 160, ratio: 2.6},
  geoOrthographicRaw: {
    n: 'Orthographic',
    arg: null,
    scale: 480,
  ...
  geoPolyconicRaw: {n: 'Polyconic', arg: null, scale: 160, ratio: 1.3},
  geoQuincuncialRaw: {n: 'Quincuncial', arg: null, scale: 160, ratio: 1.3},
  ...
  geoRobinsonRaw: {n: 'Robinson', arg: null, scale: 160},
  geoSinusoidalRaw: {n: 'Sinusoidal', arg: null, scale: 160, ratio: 2},
  geoStereographicRaw: {
    n: 'Stereographic',
    arg: null,
    scale: 500,
    ...
    fRaw: function (x, yRaw) {
      return d3.geoStereographicRaw(x, y);
    },
  },
  geoTimesRaw: {n: 'Times', arg: null, scale: 210, ratio: 1.4},
  ...
  geoWagner4Raw: {n: 'Wagner IV', arg: null, scale: 185},
  geoWagner6Raw: {n: 'Wagner VI', arg: null, scale: 160},
  geoWagner7Raw: {n: 'Wagner VII', arg: null, scale: 190, ratio: 1.8},
  geoWiechelRaw: {n: 'Wiechel', arg: null, scale: 360, ratio: 1.0, clip: true},
  geoWinkel3Raw: {n: 'Winkel Tripel', arg: null, scale: 196, ratio: 1.7},
};
...
var formats_all = {
  iau: Object.keys(formats.constellations.iau.names)
    .concat(Object.keys(formats.planets.iau.names))
    .filter(function (value, index, self) {
      return self.indexOf(value) === index;
    }),
  cn: Object.keys(formats.constellations.cn.names)
    .concat(Object.keys(formats.starnames.cn.propername))
    .filter(function (value, index, self) {
      return self.indexOf(value) === index;
    }),
};
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (
      ...
      if ((antimeridian ^ lambda0) >= (lambda ^ lambda1) >= lambda) {
        ...
        var phiArc =
          (antimeridian ^ (delta >= 0) ? -1 : 1) * Math.asin(intersection[2]);
...
function form(cfg) {
  ...
  col
    ...
    .attr('id', 'width')
    .attr('value', config.width)
    .on('change', resize);
  col.append('span').html('px');

  col
    .append('label')
    .attr('title', 'Map projection, (hemi) indicates hemispherical projection')
    .attr('for', 'projection')
    .html('Projection');
  var sel = col
    .append('select')
  ...
  var list = Object.keys(prj).map(function (key, i) {
    var n =
      prj[key].clip && prj[key].clip === true
        ? prj[key].n + ' (hemi)'
        : prj[key].n;
  ...
  col
    .append('label')
    .attr('title', 'Center coordinates long/lat in selected coordinate space')
    .attr('for', 'centerx')
    .html('Center');
  col
    .append('input')
  ...
  col
    ...
    .attr('class', 'advanced')
    .property('checked', config.orientationfixed)
    .on('change', apply);
  ...
  col
    ...
    .property('checked', config.stars.colors)
    .on('change', apply);
  ...
  col
    ...
    .property('value', config.stars.style.fill)
    .on('change', apply);
  ...
  for (var fld in names) {
    ...
    col
      .append('label')
      .attr('for', 'stars-' + fld + 'Limit')
      .html('down to mag');
    col
      .append('input')
      .attr('type', 'number')
      .attr('id', 'stars-' + fld + 'Limit')
      .attr('title', 'Star name display limit (magnitude)')
      .attr('value', config.stars[fld + 'Limit'])
      .attr('max', '6')
      .attr('min', '-1')
      .attr('step', '0.1')
      .on('change', apply);
  ...
  col
    ...
    .attr('type', 'checkbox')
    .attr('id', 'dsos-show')
    .property('checked', config.dsos.show)
    .on('change', apply);
  ...
  col
    ...
    .attr('type', 'checkbox')
    .attr('id', 'dsos-colors')
    .property('checked', config.dsos.colors)
    .on('change', apply);
  ...
  col
    ...
    .property('value', config.dsos.style.fill)
    .on('change', apply);
  ...
  for (fld in names) {
    ...
    col
      ...
      .attr('for', 'dsos-' + fld + 'Type')
      .attr('class', 'advanced')
      .html('');
    sel = col
      .append('select')
      .attr('id', 'dsos-' + fld + 'Type')
      .attr('class', 'advanced')
      .on('change', apply);
    ...
    col
      ...
      .attr('type', 'checkbox')
      .attr('id', 'dsos-' + fld)
      .property('checked', config.dsos[fld])
      .on('change', apply);
  }
  ...
  col
    ...
    .attr('value', config.dsos.nameLimit)
    .attr('max', '6')
    .attr('min', '0')
    .attr('step', '0.1')
    .on('change', apply);
  ...
  col
    ...
    .attr('value', config.dsos.exponent)
    .attr('max', '3')
    .attr('min', '-1')
    .attr('step', '0.01')
    .on('change', apply);
  ...
  for (fld in names) {
    ...
    if (nameKeys.length > 1) {
      ...
      col
        ...
        .attr('for', 'constellations-' + fld + 'Type')
        .attr('class', 'advanced')
        .html('');
      sel = col
        .append('select')
        .attr('id', 'constellations-' + fld + 'Type')
        .attr('class', 'advanced')
        .on('change', apply);
      ...
      col
        .append('input')
        .attr('type', 'checkbox')
        .attr('id', 'constellations-' + fld)
        .property('checked', config.constellations[fld])
        .on('change', apply);
  ...
  col
    ...
    .property('checked', config.constellations.lines)
    .on('change', apply);
  ...
  col
    ...
    .property('checked', config.constellations.bounds)
    .on('change', apply);
  ...
  col
    ...
    .property('checked', config.lines.graticule.show)
    .on('change', apply);
  ...
  col
    ...
    .property('checked', config.lines.equatorial.show)
    .on('change', apply);
  ...
  col
    ...
    .property('checked', config.lines.ecliptic.show)
    .on('change', apply);
  ...
  col
    ...
    .property('checked', config.lines.galactic.show)
    .on('change', apply);
  ...
  col
    ...
    .property('checked', config.lines.supergalactic.show)
    .on('change', apply);
  ...
  col
    ...
    .attr('type', 'checkbox')
    .attr('id', 'mw-show')
    .property('checked', config.mw.show)
    .on('change', apply);
  ...
  col
    ...
    .attr('value', config.mw.style.fill)
    .on('change', apply);
  ...
  col
    ...
    .attr('value', config.mw.style.opacity)
    .attr('max', '1')
    .attr('min', '0')
    .attr('step', '0.01')
    .on('change', apply);
  ...
  col
    ...
    .attr('value', config.background.fill)
    .on('change', apply);
  ...
  col
    ...
    .attr('value', config.background.opacity)
    .attr('max', '1')
    .attr('min', '0')
    .attr('step', '0.01')
    .on('change', apply);
  ...
  col
    ...
    .attr('id', 'adaptable')
    .attr('class', 'advanced')
    .property('checked', config.adaptable)
    .on('change', apply);
  ...
  function getCenter() {
    ...
    if (config.transform !== 'equatorial')
      config.center[0] = parseFloat(cx.value);
    else {
  ...
  function getFilename(ext) {
    var dateFormat = d3.timeParse('%Y%m%dT%H%M%S%Z'),
  ...
  function showCon(id) {
    ...
    if (
      !isObject(Celestial.constellations) ||
      !has(Celestial.constellations, id)
    )
      return;
  ...
  function setLanguage(lang) {
    ...
    for (var i = 0; i < keys.length; i++) {
      if (has(formats[keys[i]][config.culture].names, lang))
        config[keys[i]].namesType = lang;
      else if (has(formats[keys[i]][config.culture].names, 'desig'))
        config[keys[i]].namesType = 'desig';
      else config[keys[i]].namesType = 'name';
    }
    if (has(formats.dsonames[config.culture].names, lang))
      config.dsos.namesType = lang;
    else config.dsos.namesType = 'desig';
    if (has(formats.starnames[config.culture].propername, lang))
      config.stars.propernameType = lang;
  ...
  function update() {
    ...
    d3.selectAll(
      ...
      // geopos -> lat, lon
      if (id === 'lat' || id === 'lon') {
        if (isArray(config.geopos))
          this.value = id === 'lat' ? config.geopos[0] : config.geopos[1];
  ...
  Celestial.setLanguage = function (lang) {
    ...
    if (formats_all[config.culture].indexOf(lang) !== -1)
...
function popError(nd, err) {
  ...
  d3.select('#error')
    .html(err)
    .style({top: px(p[1] + nd.offsetHeight + 1), left: px(p[0]), opacity: 1});
  nd.focus();
}
  ...
  if (node.validity) {
    ...
    if (v.rangeOverflow || v.rangeUnderflow) {
      popError(
        ...
        node.title +
          ' must be between ' +
          (parseInt(node.min) + adj) +
          ' and ' +
          (parseInt(node.max) - adj),
      );
      return false;
    }
  } else {
...
function setCenter(ctr, trans) {
  ...
  if (trans !== 'equatorial') cx.value = ctr[0].toFixed(1);
    ...
    cx.value =
      ctr[0] < 0 ? (ctr[0] / 15 + 24).toFixed(1) : (ctr[0] / 15).toFixed(1);
...
function showAdvanced(showit) {
  ...
  d3.select(parentElement + ' ~ #celestial-form')
    .selectAll('.advanced')
    .style('display', vis);
  d3.select(parentElement + ' ~ #celestial-form')
    .selectAll('#label-propername')
    .style('display', showit ? 'none' : 'inline-block');
}
...
function setVisibility(cfg, which) {
  ...
  if (cfg.form === false && cfg.location === true) {
    d3.select(parentElement + ' ~ #celestial-form').style(
      'display',
      'inline-block',
    );
  ...
  // hide if not desired
  if (cfg.form === false)
    d3.select(parentElement + ' ~ #celestial-form').style('display', 'none');
...
function listConstellations() {
  var sel = d3
      .select(parentElement + ' ~ #celestial-form')
...
function $form(id) {
  return document.querySelector(
    parentElement + ' ~ #celestial-form' + ' #' + id,
  );
}
...
function geo(cfg) {
  ...
  var dtFormat = d3.timeParse('%Y-%m-%d %H:%M:%S'),
    zenith = [0, 0],
    geopos = [0, 0],
    date = new Date(),
    ...
    frm = d3
      .select(parentElement + ' ~ #celestial-form form')
  ...
  col
    ...
    .property('checked', config.horizon.show)
    .on('change', apply);
  //Daylight
  col
    .append('label')
  ...
  col
    ...
    .property('checked', config.daylight.show)
    .on('change', apply);
  col.append('br');
  ...
  col
    ...
    .property('checked', config.planets.show)
    .on('change', apply);
  //Planet names
  ...
  for (var fld in names) {
    ...
    if (keys.length > 1) {
      ...
      if (fld === 'names') {
        ...
        col
          .append('label')
          .attr('for', 'planets-' + fld)
          .html('names');
        col
          .append('input')
          .attr('type', 'checkbox')
          .attr('id', 'planets-' + fld)
          .property('checked', config.planets[fld])
          .on('change', apply);
      }
    }
  }
  ...
  d3.select(document).on('mousedown', function () {
    if (!hasParent(d3.event.target, 'celestial-date') && dtpick.isVisible())
      dtpick.hide();
  });
  ...
  function go() {
    ...
    if (!isNaN(lon) && !isNaN(lat)) {
      ...
      zenith = Celestial.getPoint(
        horizontal.inverse(dtc, [90, 0], geopos),
        config.transform,
      );
      zenith[2] = 0;
  ...
  function setPosition(p, settime) {
    ...
    var timestamp = Math.floor(date.getTime() / 1000),
      protocol =
        window && window.location.protocol === 'https:' ? 'https' : 'http',
      url =
        protocol +
  ...
  Celestial.skyview = function (cfg) {
    ...
    //Celestial.updateForm();
    if (valid === false)
      return {date: date, location: geopos, timezone: timeZone};
  ...
  if (
    has(config, 'formFields') &&
    (config.location === true || config.formFields.location === true)
  ) {
    d3.select(parentElement + ' ~ #celestial-form')
      .select('#location')
...
var Moon = {
  elements: function (dat) {
    ...
    var sapp =
      -0.55 * Math.cos(deg2rad * (238.2 + 854535.2 * t)) +
      0.1 * Math.cos(deg2rad * (103.2 + 377336.3 * t)) +
      0.1 * Math.cos(deg2rad * (233.2 + 926533.3 * t));
    ...
    var sgp =
      -0.0203 * Math.cos(deg2rad * (125.0 - 1934.1 * t)) +
    ...
    var srp =
      25.9 * Math.sin(deg2rad * (125.0 - 1934.1 * t)) -
    ...
    var slp =
      3.96 * Math.sin(deg2rad * (119.7 + 131.8 * t)) +
      1.96 * Math.sin(deg2rad * (125.0 - 1934.1 * t));

    var slpp =
      0.463 * Math.sin(deg2rad * (357.5 + 35999.1 * t)) +
    ...
    var slppp =
      14 * Math.sin(deg2rad * (357.5 + 35999.1 * t)) +
...
function exportSVG(fname) {
  var doc = d3
    ...
    projection = Celestial.projection(cfg.projection)
      .rotate(rotation)
  ...
  circle = d3.geoCircle().center([0, 0]).radius([179.95]).origin(center);
  ...
  for (var key in cfg.lines) {
    if (
      has(cfg.lines, key) &&
      key != 'graticule' &&
      cfg.lines[key].show !== false
    ) {
      id = 'planes' + key;
      groups[id]
        .append('path')
        .datum(d3.geoCircle().center([0, 0]).radius([90])().origin(poles[key]))
        .attr('class', id)
        .attr('d', map);
    ...
    q.defer(function (callback) {
      d3.json(
        ...
        function (error, json) {
          ...
          groups.constBoundaries
            .selectAll('.bounds')
            .data(conb.features)
            .enter()
            .append('path')
            .attr('class', function (d) {
              return Celestial.constellation && d.ids.search(re) !== -1
                ? 'constBoundariesSel'
                : 'constBoundaries';
            })
            .attr('d', map);
  ...
  if (cfg.constellations.lines) {
    q.defer(function (callback) {
      d3.json(
        path + filename('constellations', 'lines'),
        function (error, json) {
          if (error) callback(error);
          ...
          var dasharray = has(cfg.constellations.lineStyle, 'dash')
            ? cfg.constellations.lineStyle.dash.join(' ')
            : 'none';
    ...
    q.defer(function (callback) {
      d3.json(path + filename('constellations'), function (error, json) {
        ...
        groups.constNames
          ...
          .data(
            conn.features.filter(function (d) {
    ...
    q.defer(function (callback) {
      d3.json(path + cfg.stars.data, function (error, json) {
        ...
        groups.stars
          .selectAll('.stars')
          .data(
            cons.features.filter(function (d) {
        ...
        if (cfg.stars.designation) {
          groups.starDesignations
            .selectAll('.stardesigs')
            .data(
              cons.features.filter(function (d) {
                return (
                  d.properties.mag <= cfg.stars.designationLimit * adapt &&
                  clip(d.geometry.coordinates) === 1
                );
              }),
            )
            .enter()
    ...
    q.defer(function (callback) {
      d3.json(path + cfg.dsos.data, function (error, json) {
        ...
        groups.dsos
          .selectAll('.dsos')
          .data(
            cond.features.filter(function (d) {
              return (
                clip(d.geometry.coordinates) === 1 &&
                ((d.properties.mag === 999 &&
                  Math.sqrt(parseInt(d.properties.dim)) >
                    cfg.dsos.limit * adapt) ||
                  (d.properties.mag !== 999 &&
                    d.properties.mag <= cfg.dsos.limit))
              );
            }),
          )
          .enter()
        ...
        for (key in cfg.dsos.symbols) {
          ...
          if (has(cfg.dsos.symbols[key], 'stroke')) {
            styles[id].fill = 'none';
            styles[id].stroke = cfg.dsos.colors
              ? cfg.dsos.symbols[key].stroke
              : cfg.dsos.style.stroke;
            styles[id]['stroke-width'] = cfg.dsos.colors
              ? cfg.dsos.symbols[key].width
              : cfg.dsos.style.width;
          } else {
            styles[id].stroke = 'none';
            styles[id].fill = cfg.dsos.colors
              ? cfg.dsos.symbols[key].fill
              : cfg.dsos.style.fill;
          }
        }
                ...
                return (
                  ...
                  ((d.properties.mag == 999 &&
                    Math.sqrt(parseInt(d.properties.dim)) >
                      cfg.dsos.nameLimit) ||
                    (d.properties.mag != 999 &&
                      d.properties.mag <= cfg.dsos.nameLimit))
                );
              }),
            )
            .enter()
    ...
    q.defer(function (callback) {
      ...
      if (jlun.features.length > 0) {
        if (cfg.planets.symbolType === 'letter') {
          ...
          var rl = has(cfg.planets.symbols.lun, 'size')
            ? (cfg.planets.symbols.lun.size - 1) * adapt
            : 11 * adapt;
          groups.planets
            .selectAll('.dmoon')
            .data(jlun.features)
  ...
  function skyTransparency(t, a) {
    return (
      0.9 * (1 - (Math.pow(Math.E, t * a) - 1) / (Math.pow(Math.E, a) - 1))
    );
  }
  ...
  function dsoSize(mag, dim) {
    if (!mag || mag === 999)
      return Math.pow((parseInt(dim) * cfg.dsos.size * adapt) / 7, 0.5);
  ...
  function createEntry(o) {
    ...
    if (
      cfg.planets.symbolType === 'symbol' ||
      cfg.planets.symbolType === 'letter'
    )
      res.properties.symbol =
        cfg.planets.symbols[res.id][cfg.planets.symbolType];
...
var customSvgSymbols = new Map();
customSvgSymbols.set('ellipse', function (size, ratio) {
  var s = Math.sqrt(size),
    rx = s * 0.666,
    ry = s / 3;
  return (
...
customSvgSymbols.set('marker', function (size, ratio) {
...
customSvgSymbols.set('cross-circle', function (size, ratio) {
  var s = Math.sqrt(size),
    r = s / 2;
  return (
    'M' +
...
customSvgSymbols.set('stroke-circle', function (size, ratio) {
  var s = Math.sqrt(size),
    r = s / 2;
  return (
    'M' +
...
customSvgSymbols.set('crescent', function (size, ratio) {
  var s = Math.sqrt(size),
    r = s / 2,
  ...
  this.show = function (dt, tz) {
    ...
    if (nd.offsetTop === -9999) {
      ...
      d3.select(parentElement + ' ~ #celestial-form')
        ...
        .style({top: px(top), left: px(left), opacity: 1});
      d3.select(parentElement + ' ~ #celestial-form')
        .select('#datepick')
        .classed('active', true);
    } else {
...
//       var between = rotateBetween(
//         cartesian(location),
//         cartesian([-view.r[0], -view.r[1]]),
//       );
...
//                 i = interpolateBetween(
//                   quaternionFromEuler(view.r),
//                   quaternionFromEuler(view1.r),
//                 ),
//                 d = d3.geoDistance(view.r, view1.r),
//                 smooth = d3.interpolateZoom(
//                   [0, 0, width / view.k],
//                   [d, 0, width / view1.k],
//                 );
//               if (duration)
//                 transition.duration(duration(smooth.duration * 0.001)); // see https://github.com/mbostock/d3/pull/2045
//               return function (t) {
//                 var uw = smooth(t);
//                 this.__chart__ = view = {
...
//       angle =
//         Math.atan2(p0[1] - t[1], p0[0] - t[0]) -
//         Math.atan2(p1[1] - t[1], p1[0] - t[0]);
