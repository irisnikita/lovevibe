...
var {geoZoom} = require('./lib/d3-geo-zoom');
...
!(function () {
  ...
  Celestial.display = function (config) {
    ...
    function reproject(config) {
      ...
      if (
        projectionSetting.clip != prj.clip ||
        cfg.disableAnimations === true
      ) {
    ...
    function redraw() {
      ...
      if (cfg.constellations.bounds) {
        container
          .selectAll(parentElement + ' .boundaryline')
          .each(function (d) {
    ...
    function clip(coords) {
      return projectionSetting.clip &&
        d3.geo.distance(cfg.center, coords) > halfπ
        ? 0
        : 1;
    }
    ...
    function clear() {
      context.clearRect(
        0,
        0,
        canvaswidth + margin[0],
        canvasheight + margin[1],
      );
    }
  ...
  */
  ...
  var euler = {
    ...
    add: function (name, ang) {
      if (!ang || !name || ang.length !== 3 || this.hasOwnProperty(name))
        return;
  ...
  var settings = {
    ...
    set: function (cfg) {
      ...
      var prop,
        ...
        config = {},
        res = {};
      if (Object.entries(globalConfig).length === 0)
      ...
      for (prop in config) {
        ...
        if (!has(cfg, prop) || cfg[prop] === null) {
          ...
          config[prop] === null ||
          config[prop].constructor != Object
        ) {
          res[prop] = cfg[prop];
        } else {
          res[prop] = {};
    ...
    applyDefaults: function (cfg) {
      ...
      if (!res.culture || res.culture.search(/^cn$/) === -1)
        res.culture = 'iau';
      ...
      if (cfg) {
        ...
        if (has(cfg, 'dsos')) {
          ...
          if (has(cfg.dsos, 'namelimit'))
            res.dsos.nameLimit = cfg.dsos.namelimit;
      ...
      if (
        !has(
          formats.starnames[res.culture].propername,
          res.stars.propernameType,
        )
      )
...
*/
  ...
  function longitude(point) {
    if (Math.abs(point[0]) <= Math.PI) return point[0];
      ...
      return (
        sign(point[0]) * (((Math.abs(point[0]) + Math.PI) % tau) - Math.PI)
      );
  }
  ...
  function form(cfg) {
    ...
    names =
      formats.constellations[config.culture] || formats.constellations.iau;
  ...
  function enable(source) {
    ...
    switch (fld) {
      ...
      case 'stars-designation':
        off =
          !$form('stars-designation').checked || !$form('stars-show').checked;
      ...
      case 'stars-propername':
        off =
          !$form('stars-propername').checked || !$form('stars-show').checked;
  ...
  function geo(cfg) {
    ...
    function here() {
      navigator.geolocation.getCurrentPosition(function (pos) {
        geopos = [
          Round(pos.coords.latitude, 4),
          Round(pos.coords.longitude, 4),
        ];
  ...
  var Kepler = function () {
    ...
    function magnitude() {
      var de = dat.ephemeris,
        ...
        q =
          0.666 *
          ((1 - a / Math.PI) * Math.cos(a) + (1 / Math.PI) * Math.sin(a)),
        m =
          dat.H * 1 +
    ...
    if (cfg.mw.show) {
      q.defer(function (callback) {
        d3.json(path + 'mw.json', function (error, json) {
          ...
          if (
            !has(cfg.background, 'opacity') ||
            cfg.background.opacity > 0.95
          ) {
            groups.milkyWayBg
              .selectAll('.mwaybg')
              .data(mw_back.features)
          ...
          .on('zoomstart', function () {
            ...
            zoomOn.call(zoom, 'zoom', function () {
              ...
              var mouse1 = d3.mouse(this),
                between = rotateBetween(
                  zoomPoint,
                  position(projection, mouse1),
                );
              projection.rotate(
