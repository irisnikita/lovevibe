(function () {
  d3.geo.project = function (object, projection) {
    ...
    return (
      object && d3_geo_projectObjectType.hasOwnProperty(object.type)
        ? d3_geo_projectObjectType[object.type]
        : d3_geo_projectGeometry
    )(object, stream);
  };
  ...
  var d3_geo_projectLine = {
    ...
    lineEnd: function () {
      if (d3_geo_projectPoints.length)
        d3_geo_projectLines.push(d3_geo_projectPoints),
          (d3_geo_projectPoints = []);
    },
    result: function () {
  ...
  var d3_geo_projectPolygon = {
    ...
    lineEnd: function () {
      ...
      if (n) {
        do d3_geo_projectPoints.push(d3_geo_projectPoints[0].slice());
        while (++n < 4);
        d3_geo_projectLines.push(d3_geo_projectPoints),
          (d3_geo_projectPoints = []);
      }
  ...
  function airy(β) {
    var tanβ_2 = Math.tan(0.5 * β),
      B = (2 * Math.log(Math.cos(0.5 * β))) / (tanβ_2 * tanβ_2);
    function forward(λ, φ) {
      var cosλ = Math.cos(λ),
        cosφ = Math.cos(φ),
        sinφ = Math.sin(φ),
        cosz = cosφ * cosλ,
        K = -(
          (1 - cosz ? Math.log(0.5 * (1 + cosz)) / (1 - cosz) : -0.5) +
          B / (1 + cosz)
        );
    ...
    forward.invert = function (x, y) {
      ...
      do {
        ...
        z -= δ =
          ((2 / tanz_2) * lnsecz_2 - B * tanz_2 - ρ) /
          (-lnsecz_2 / (sinz_2 * sinz_2) + 1 - B / (2 * cosz_2 * cosz_2));
  ...
  function armadillo(φ0) {
    ...
    function forward(λ, φ) {
      ...
      return [
        (1 + cosφ) * Math.sin(λ),
        (sφ0 * φ > -Math.atan2(cosλ, tanφ0) - 0.001 ? 0 : -sφ0 * 10) +
          k +
          Math.sin(φ) * cosφ0 -
          (1 + cosφ) * sinφ0 * cosλ,
      ];
    ...
    forward.invert = function (x, y) {
      ...
      do {
        var cosλ = Math.cos(λ),
          ...
          δxδλ = 0.5 * A * cosλ,
          δxδφ = -sinλ * sinφ,
          δyδλ = 0.5 * sinφ0 * A * sinλ,
          δyδφ = cosφ0 * cosφ + sinφ0 * cosλ * sinφ,
          denominator = δxδφ * δyδλ - δyδφ * δxδλ,
          δλ = (0.5 * (fy * δxδφ - fx * δyδφ)) / denominator,
          δφ = (fx * δyδλ - fy * δxδλ) / denominator;
        (λ -= δλ), (φ -= δφ);
      } while ((Math.abs(δλ) > ε || Math.abs(δφ) > ε) && --i > 0);
      return sφ0 * φ > -Math.atan2(Math.cos(λ), tanφ0) - 0.001
        ? [λ * 2, φ]
        : null;
    };
    return forward;
    ...
    p.stream = function (stream) {
      ...
      rotateStream.sphere = function () {
        ...
        while (sφ0 * (λ -= φ0) >= -180) {
          sphereStream.point(
            λ,
            sφ0 * -Math.atan2(Math.cos((λ * radians) / 2), tanφ0) * degrees,
          );
        }
  ...
  august.invert = function (x, y) {
    ...
    var x2 = x * x,
      y2 = y * y,
      s = 1 + x2 + y2,
      sin3η = Math.sqrt(0.5 * (s - Math.sqrt(s * s - 4 * y * y))),
      η = asin(sin3η) / 3,
    ...
    return [
      sgn(x) * 2 * Math.atan2(sinh(ξ) * cosη, 0.25 - d),
      sgn(y) * 2 * Math.atan2(coshξ * Math.sin(η), 0.25 + d),
    ];
  };
  ...
  baker.invert = function (x, y) {
    ...
    do {
      var cosφ_2 = Math.cos(φ / 2),
        tanφ_2 = Math.tan(φ / 2);
      φ -= δ =
        (sqrt8 * (φ - π / 4) - Math.log(tanφ_2) - y0) /
        (sqrt8 - (0.5 * cosφ_2 * cosφ_2) / tanφ_2);
      ...
      if (r > halfπ) {
        var θ = Math.atan2(y, x),
          ...
          cotα =
            1 /
            Math.tan(
              s * Math.acos((A - π) / Math.sqrt(π * (π - 2 * A) + r * r)),
            );
    ...
    p.stream = function (stream) {
      ...
      rotateStream.sphere = function () {
        ...
        for (
          ...
          sphereStream.point(
            Math.atan2(sr * Math.cos(φ0), cr) * degrees,
            asin(sr * Math.sin(φ0)) * degrees,
          );
          if (φ < -90) {
  ...
  boggs.invert = function (x, y) {
    ...
    do {
      ...
      θ -= δ =
        (Math.sin(2 * θ) + 2 * θ - π * Math.sin(φ)) /
        (2 * Math.cos(2 * θ) + 2 + π * Math.cos(φ) * Math.SQRT2 * Math.cos(θ));
    } while (Math.abs(δ) > ε && --i > 0);
    φ = ky - Math.SQRT2 * Math.sin(θ);
  ...
  function chamberlinDistanceAzimuth(dφ, c1, s1, c2, s2, dλ) {
    ...
    if (Math.abs(dφ) > 1 || Math.abs(dλ) > 1) {
      ...
      var sindφ = Math.sin(0.5 * dφ),
        sindλ = Math.sin(0.5 * dλ);
  ...
  function craig(φ0) {
    ...
    function forward(λ, φ) {
      return [
        λ,
        (λ ? λ / Math.sin(λ) : 1) *
          (Math.sin(φ) * Math.cos(λ) - tanφ0 * Math.cos(φ)),
      ];
    }
    forward.invert = tanφ0
      ? function (x, y) {
          if (x) y *= Math.sin(x) / x;
          var cosλ = Math.cos(x);
          return [
            ...
            2 *
              Math.atan2(
                Math.sqrt(cosλ * cosλ + tanφ0 * tanφ0 - y * y) - cosλ,
                tanφ0 - y,
              ),
          ];
        }
  ...
  function cylindricalStereographic(φ0) {
    var cosφ0 = Math.cos(φ0);
    function forward(λ, φ) {
      return [λ * cosφ0, (1 + cosφ0) * Math.tan(φ * 0.5)];
    }
    forward.invert = function (x, y) {
  ...
  function eisenlohr(λ, φ) {
    var s0 = Math.sin((λ /= 2)),
      ...
      c = Math.sqrt(2 / (1 + t * t)),
      v = Math.sqrt(
        (Math.SQRT2 * c1 + (c0 + s0) * k) / (Math.SQRT2 * c1 + (c0 - s0) * k),
      );
    return [
  ...
  eisenlohr.invert = function (x, y) {
    ...
    do {
      var _0 = λ / 2,
        ...
        δtδφ =
          (Math.SQRT2 * c0 * c1 + k) /
          (2 * (c1 + Math.SQRT2 * c0 * k) * (c1 + Math.SQRT2 * c0 * k) * k),
        δcδt = -0.5 * t * c * c * c,
        δcδλ = δcδt * δtδλ,
        δcδφ = δcδt * δtδφ,
  ...
  foucaut.invert = function (x, y) {
    var k = Math.atan(y / sqrtπ),
      cosk = Math.cos(k),
      φ = 2 * k;
    return [(x * sqrtπ * 0.5) / (Math.cos(φ) * cosk * cosk), φ];
  };
  ...
  d3.geo.gilbert = function (projection) {
    ...
    function gilbert(coordinates) {
      return projection([
        coordinates[0] * 0.5,
        asin(Math.tan(coordinates[1] * 0.5 * radians)) * degrees,
      ]);
    }
    if (projection.invert)
      gilbert.invert = function (coordinates) {
        coordinates = projection.invert(coordinates);
        coordinates[0] *= 2;
        coordinates[1] =
          2 * Math.atan(Math.sin(coordinates[1] * radians)) * degrees;
        return coordinates;
      };
    gilbert.stream = function (stream) {
      ...
      var s = e.stream({
        point: function (λ, φ) {
          stream.point(λ * 0.5, asin(Math.tan(-φ * 0.5 * radians)) * degrees);
        },
        lineStart: function () {
          stream.lineStart();
        },
          ...
          α -= δ =
            ...
            (sinx -
              (x_halfπ *
                2 *
                (halfπ_ρcosα *
                  (ρcosα + α * ρsinα * Math.cos(ρcosα) - sinρcosα) -
                  ρsinα * (ρsinα - α * sinρcosα))) /
                (halfπ_ρcosα * halfπ_ρcosα));
        }
        r =
          ρ +
          (gingeryIntegrate(s_, ρcosα, x) * (π - ρ)) /
            gingeryIntegrate(s_, ρcosα, π);
        θ = θ0 + α;
        x = r * Math.cos(θ);
        y = r * Math.sin(θ);
      }
    ...
    p.stream = function (stream) {
      ...
      rotateStream.sphere = function () {
        ...
        for (var i = 0, δ = (2 * π) / n, φ = 0; i < n; ++i, φ -= δ) {
          sphereStream.point(
            Math.atan2(sr * Math.cos(φ), cr) * degrees,
            Math.asin(sr * Math.sin(φ)) * degrees,
          );
          sphereStream.point(
            Math.atan2(sρ * Math.cos(φ - δ / 2), cρ) * degrees,
            Math.asin(sρ * Math.sin(φ - δ / 2)) * degrees,
          );
        }
        sphereStream.lineEnd(), sphereStream.polygonEnd();
      };
    ...
    forward.invert = function (x, y) {
      ...
      do {
        ...
        (xB2 = xB * xB),
          ...
          (msinα = m * sinα),
          (dαdφ =
            ((λ / π) * (1 / asqrt(1 - xB2 / m2)) * (dxBdφ * m - xB * dmdφ)) /
            m2),
          (fx = msinα - x),
  ...
  var ginzburg4 = ginzburgPolyconic(
    2.8284,
    -1.6988,
    0.75432,
    -0.18071,
  ...
  var ginzburg5 = ginzburgPolyconic(
    2.583819,
    -0.835827,
    0.170354,
    -0.038094,
  ...
  var ginzburg6 = ginzburgPolyconic(
    (5 / 6) * π,
    -0.62636,
    -0.0344,
    0,
  ...
  function ginzburg8(λ, φ) {
    var λ2 = λ * λ,
      φ2 = φ * φ;
    return [
      λ * (1 - 0.162388 * φ2) * (0.87 - 952426e-9 * λ2 * λ2),
      φ * (1 + φ2 / 12),
    ];
  }
  ginzburg8.invert = function (x, y) {
    ...
    do {
      var λ4 = (λ4 = λ * λ) * λ4;
      λ -= δ = (λ * (0.87 - 952426e-9 * λ4) - x) / (0.87 - 0.00476213 * λ4);
    } while (Math.abs(δ) > ε && --i > 0);
    return [λ, φ];
  };
  ...
  var ginzburg9 = ginzburgPolyconic(
    2.6516,
    -0.76534,
    0.19123,
    -0.047094,
  ...
  function quincuncialProjection(projectHemisphere) {
    ...
    function projectAt(quincuncial) {
      ...
      if (projectHemisphere.invert)
        forward.invert = quincuncial
          ? function (x0, y0) {
  ...
  function ellipticJi(u, v, m) {
    ...
    return [
      ...
      [
        (a[2] * b[1] * b[2]) / denominator,
        (-m * a[0] * a[1] * b[0]) / denominator,
      ],
    ];
  }
  function ellipticJ(u, m) {
    ...
    if (m < ε) {
      ...
      b = Math.cos(u);
      ai = 0.25 * m * (u - t * b);
      return [t - ai * b, b + ai * t, 1 - 0.5 * m * t * t, u - ai];
    }
    if (m >= 1 - ε) {
      ai = 0.25 * (1 - m);
      b = cosh(u);
  ...
  function guyouComplexAtan(x, y) {
    var x2 = x * x,
      ...
      t = 1 - x2 - y * y;
    return [
      0.5 * ((x >= 0 ? halfπ : -halfπ) - Math.atan2(t, 2 * x)),
  ...
  guyou.invert = function (x, y) {
    ...
    return [
      ...
      2 *
        Math.atan(
          Math.exp(
            (0.5 / f) * Math.log(k_ * tn[0] * tn[0] + k_ * tn[1] * tn[1]),
          ),
        ) -
        halfπ,
    ];
  ...
  function hammerRetroazimuthal(φ0) {
    ...
    function forward(λ, φ) {
      ...
      return [
        K * cosφ0 * Math.sin(λ),
        (Math.abs(λ) > halfπ ? K : -K) * (sinφ0 * cosφ - cosφ0 * sinφ * cosλ),
      ];
    }
    forward.invert = function (x, y) {
      var ρ = Math.sqrt(x * x + y * y),
        ...
        λ =
          (ρ > halfπ ? -1 : 1) *
          Math.atan2(x * sinz, ρ * Math.cos(φ) * cosz + y * Math.sin(φ) * sinz);
  ...
  function hammerRetroazimuthalProjection() {
    ...
    p.rotate = function (_) {
      if (!arguments.length)
        return (_ = rotate_.call(p)), (_[1] += (φ0 / π) * 180), _;
  ...
  (d3.geo.hammerRetroazimuthal = hammerRetroazimuthalProjection).raw =
  ...
  function hatano(λ, φ) {
    ...
    return [
      0.85 * λ * Math.cos((φ *= 0.5)),
      Math.sin(φ) * (φ < 0 ? 1.93052 : 1.75859),
    ];
  }
  hatano.invert = function (x, y) {
    var θ =
      Math.abs((θ = y * (y < 0 ? 0.5179951515653813 : 0.5686373742600607))) >
      1 - ε
        ? θ > 0
          ? halfπ
          : -halfπ
        : asin(θ);
    return [
      (1.1764705882352942 * x) / Math.cos(θ),
      Math.abs(
        (θ =
          ((θ += θ) + Math.sin(θ)) *
          (y < 0 ? 0.4102345310814193 : 0.3736990601468637)),
      ) >
      1 - ε
        ? θ > 0
          ? halfπ
  ...
  function healpix(h) {
    ...
    function forward(λ, φ) {
      ...
      if (φ2 > φ0) {
        ...
        point[0] =
          (point[0] * dx0) / dx1 - (dx0 * (h - 1)) / (2 * h) + (i * dx0) / h;
    ...
    forward.invert = function (x, y) {
      ...
      if (y2 > y0) {
        ...
        var point = d3.geo.collignon.raw.invert(
          x,
          (0.25 * (y2 - y0) * dx0) / dy1 + y1,
        );
        point[0] -= (π * (h - 1)) / h - i * k;
  ...
  function healpixProjection() {
    ...
    function sphere() {
      ...
      return {
        ...
        coordinates: [
          d3
            .range(-180, 180 + step / 2, step)
            .map(function (x, i) {
              return [x, i & 1 ? 90 - 1e-6 : healpixParallel];
            })
            .concat(
              d3.range(180, -180 - step / 2, -step).map(function (x, i) {
                return [x, i & 1 ? -90 + 1e-6 : -healpixParallel];
              }),
            ),
        ],
  ...
  function hill(K) {
    ...
    forward.invert = function (x, y) {
      ...
      return [
        ...
        asin(
          1 -
            (2 * (θ - K2 * β - L * sinθ + (1 + L2 - 2 * L * cosθ) * β_β1)) / B,
        ),
      ];
    };
    return forward;
  ...
  var sinuMollweideφ = 0.7109889596207567,
  ...
  function homolosine(λ, φ) {
    return Math.abs(φ) > sinuMollweideφ
      ? ((λ = mollweide(λ, φ)),
        (λ[1] -= φ > 0 ? sinuMollweideY : -sinuMollweideY),
        λ)
      : sinusoidal(λ, φ);
  }
  ...
  larrivee.invert = function (x, y) {
    ...
    for (var i = 0; i < 25; i++) {
      var sinφ = Math.sin(φ),
        ...
        sinλ_6 = Math.sin(λ / 6),
        cosλ_6 = Math.cos(λ / 6),
        f0 = 0.5 * λ * (1 + sqrtcosφ) - x0,
        f1 = φ / (cosφ_2 * cosλ_6) - y0,
        df0dφ = sqrtcosφ ? (-0.25 * λ * sinφ) / sqrtcosφ : 0,
        df0dλ = 0.5 * (1 + sqrtcosφ),
        df1dφ = (1 + (0.5 * φ * sinφ_2) / cosφ_2) / (cosφ_2 * cosλ_6),
  ...
  function laskowski(λ, φ) {
    var λ2 = λ * λ,
      φ2 = φ * φ;
    return [
      λ * (0.975534 + φ2 * (-0.119161 + λ2 * -0.0143059 + φ2 * -0.0547009)),
      φ *
        (1.00384 +
          λ2 * (0.0802894 + φ2 * -0.02855 + λ2 * 199025e-9) +
          φ2 * (0.0998909 + φ2 * -0.0491032)),
    ];
  }
  laskowski.invert = function (x, y) {
    ...
    do {
      var λ2 = λ * λ,
        ...
        fx =
          λ *
            (0.975534 + φ2 * (-0.119161 + λ2 * -0.0143059 + φ2 * -0.0547009)) -
          x,
        fy =
          φ *
            (1.00384 +
              ...
              φ2 * (0.0998909 + φ2 * -0.0491032)) -
          y,
        δxδλ = 0.975534 - φ2 * (0.119161 + 3 * λ2 * 0.0143059 + φ2 * 0.0547009),
        δxδφ = -λφ * (2 * 0.119161 + 4 * 0.0547009 * φ2 + 2 * 0.0143059 * λ2),
        δyδλ = λφ * (2 * 0.0802894 + 4 * 199025e-9 * λ2 + 2 * -0.02855 * φ2),
        δyδφ =
          1.00384 +
          λ2 * (0.0802894 + 199025e-9 * λ2) +
          φ2 * (3 * (0.0998909 - 0.02855 * λ2) - 5 * 0.0491032 * φ2),
        denominator = δxδφ * δyδλ - δyδφ * δxδλ,
  ...
  littrow.invert = function (x, y) {
    var x2 = x * x,
      ...
      cosφ = x
        ? Math.SQRT1_2 *
          Math.sqrt(
            (y2_1 - Math.sqrt(x2 * x2 + 2 * x2 * (y2 - 1) + y2_1 * y2_1)) / x2 +
              1,
          )
        : 1 / Math.sqrt(y2_1);
    ...
    function forward(λ, φ) {
      var y = φ - φ0,
        x =
          Math.abs(y) < ε
            ? λ * cosφ0
            : Math.abs((x = π / 4 + φ / 2)) < ε ||
              Math.abs(Math.abs(x) - halfπ) < ε
            ? 0
            : (λ * y) / Math.log(Math.tan(x) / tanφ0);
      return [x, y];
    }
    forward.invert = function (x, y) {
      var λ,
      ...
      return [
        Math.abs(y) < ε
          ? x / cosφ0
          : Math.abs((λ = π / 4 + φ / 2)) < ε ||
            Math.abs(Math.abs(λ) - halfπ) < ε
          ? 0
    ...
    forward.invert = function (x, y) {
      ...
      if (i) {
        var ρ = Math.sqrt(zr * zr + zi * zi),
          ...
          sinc = Math.sin(c);
        return [
          Math.atan2(zr * sinc, ρ * Math.cos(c)),
          ρ ? asin((zi * sinc) / ρ) : 0,
        ];
      }
    };
  ...
  function modifiedStereographicProjection() {
    ...
    p.coefficients = function (_) {
      ...
      return m(
        (coefficients =
          typeof _ === 'string' ? modifiedStereographicCoefficients[_] : _),
      );
    };
    return p;
  }
  (d3.geo.modifiedStereographic = modifiedStereographicProjection).raw =
  ...
  function mtFlatPolarQuartic(λ, φ) {
    ...
    for (var i = 0, δ; i < 25; i++) {
      θ -= δ =
        (Math.sin(θ / 2) + Math.sin(θ) - k) /
        (0.5 * Math.cos(θ / 2) + Math.cos(θ));
      if (Math.abs(δ) < ε) break;
    }
    return [
  ...
  function mtFlatPolarSinusoidal(λ, φ) {
    var A = Math.sqrt(6 / (4 + π)),
      ...
      θ = φ / 2;
    for (var i = 0, δ; i < 25; i++) {
      θ -= δ = (θ / 2 + Math.sin(θ) - k) / (0.5 + Math.cos(θ));
      if (Math.abs(δ) < ε) break;
    }
    return [(A * (0.5 + Math.cos(θ)) * λ) / 1.5, A * θ];
  }
  ...
  nellHammer.invert = function (x, y) {
    ...
    for (var i = 0, δ = Infinity; i < 10 && Math.abs(δ) > ε; i++) {
      var c = Math.cos(y / 2);
      y -= δ = (y - Math.tan(y / 2) - p) / (1 - 0.5 / (c * c));
    }
  ...
  function patterson(λ, φ) {
    ...
    return [
      ...
      φ *
        (pattersonK1 +
          φ2 * φ2 * (pattersonK2 + φ2 * (pattersonK3 + pattersonK4 * φ2))),
    ];
  }
  patterson.invert = function (x, y) {
    ...
    do {
      ...
      yc -= δ =
        (yc *
          (pattersonK1 +
            y2 * y2 * (pattersonK2 + y2 * (pattersonK3 + pattersonK4 * y2))) -
          y) /
        (pattersonC1 +
  ...
  (d3.geo.peirceQuincuncial = function () {
    return peirceQuincuncialProjection()
      .quincuncial(true)
      .rotate([-90, -90, 45])
      .clipAngle(180 - 1e-6);
  ...
  polyconic.invert = function (x, y) {
    ...
    do {
      var tanφ = Math.tan(φ),
        ...
        j = k - 2 * y * φ + φ * φ;
      φ -= δ =
        (tanφ * j + 2 * (φ - y)) / (2 + j * secφ * secφ + 2 * (φ - y) * tanφ);
    } while (Math.abs(δ) > ε && --i > 0);
    tanφ = Math.tan(φ);
    return [
      (Math.abs(y) < Math.abs(φ + 1 / tanφ)
        ? asin(x * tanφ)
        : sgn(x) * (acos(Math.abs(x * tanφ)) + halfπ)) / Math.sin(φ),
      φ,
    ];
  };
  ...
  function rectangularPolyconic(φ0) {
    ...
    forward.invert = function (x, y) {
      if (Math.abs((y += φ0)) < ε)
        return [sinφ0 ? (2 * Math.atan((sinφ0 * x) / 2)) / sinφ0 : x, 0];
      var k = x * x + y * y,
        φ = 0,
        i = 10,
        δ;
  ...
  function times(λ, φ) {
    var t = Math.tan(φ / 2),
      s = Math.sin((π / 4) * t);
    return [λ * (0.74482 - 0.34588 * s * s), 1.70711 * t];
  }
  times.invert = function (x, y) {
    var t = y / 1.70711,
      s = Math.sin((π / 4) * t);
    return [x / (0.74482 - 0.34588 * s * s), 2 * Math.atan(t)];
  };
  ...
  function twoPointEquidistant(z0) {
    ...
    function forward(λ, φ) {
      ...
      return [
        (za - zb) / (2 * z0),
        (ys * asqrt(4 * z02 * zb - (z02 - za + zb) * (z02 - za + zb))) /
          (2 * z0),
      ];
    }
    forward.invert = function (x, y) {
  ...
  (d3.geo.twoPointEquidistant = twoPointEquidistantProjection).raw =
    twoPointEquidistant;
  ...
  (d3.geo.twoPointAzimuthal = twoPointAzimuthalProjection).raw =
    twoPointAzimuthal;
  function vanDerGrinten(λ, φ) {
    if (Math.abs(φ) < ε) return [λ, 0];
    var sinθ = Math.abs(φ / halfπ),
      θ = asin(sinθ);
    if (Math.abs(λ) < ε || Math.abs(Math.abs(φ) - halfπ) < ε)
      return [0, sgn(φ) * π * Math.tan(θ / 2)];
    var cosθ = Math.cos(θ),
    ...
    return [
      (sgn(λ) *
        π *
        (A * G_P2 + Math.sqrt(A2 * G_P2 * G_P2 - P2_A2 * (G * G - P2)))) /
        P2_A2,
  ...
  function vanDerGrinten4(λ, φ) {
    ...
    var B = φ0 / halfπ,
      ...
      F =
        B_C2 * (B2 + C2 * D2 - 1) +
        (1 - B2) * (B2 * (B_3C * B_3C + 4 * C2) + 12 * BC * C2 + 4 * C2 * C2),
  ...
  vanDerGrinten4.invert = function (x, y) {
    ...
    do {
      var B2 = B * B,
        ...
        F =
          B_C2 * (B2 + C2 * D2 - 1) +
          (1 - B2) * (B2 * (B_3C * B_3C + 4 * C2) + C2 * (12 * BC + 4 * C2)),
        F_ =
          -2 *
          B_C *
          (4 * BC * C2 +
            (1 - 4 * B2 + 3 * B2 * B2) * (1 + C_) +
            C2 * (-6 + 14 * B2 - D2 + (-8 + 8 * B2 - 2 * D2) * C_) +
            BC * (-8 + 12 * B2 + (-10 + 10 * B2 - D2) * C_)),
        sqrtF = Math.sqrt(F),
  ...
  cassini.invert = function (x, y) {
    return [
      Math.asin(Math.cos(x) * Math.sin(y)),
      Math.atan2(Math.tan(x), Math.cos(y)),
    ];
  };
